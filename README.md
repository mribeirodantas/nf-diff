...
The default report is a single, standalone HTML document — no external assets, no network access — that you can open in a browser or email to a colleague. It is **paginated**: a vertical sidebar navigates the layers one page at a time (with Previous/Next controls and deep-linkable section anchors) instead of one long scroll, and the performance-regressions page leads with a diverging-bar chart of the biggest movers. For scripting and CI, `--format=json` emits the same comparison as machine-readable JSON, `--format=md` produces Markdown you can drop straight into a pull-request comment, and `--fail-on-change` turns a difference into a non-zero exit code.
