...
For authoritative process wiring and runtime metadata, enable Nextflow lineage for both runs. Create a config file such as `lineage.config` with the following content, then pass it with `-c lineage.config` to every `nextflow run` used for the comparison:

```groovy
lineage.enabled = true
```

The `nf-diff` action cannot enable lineage after the pipeline has finished. When using the cache-backed workflow below, cache the `.lineage` directory together with `.nextflow` so the baseline lineage store is available to the PR run.
